...
template <typename T> class nolock_queue {
public:
...
template <typename T> inline nolock_queue<T>::nolock_queue(size_t size = 2048)
{
    m_size = size;
...
template <typename T> inline nolock_queue<T>::~nolock_queue()
{
    if (m_queue)
    {
        delete[] m_queue;
...
template <typename T> inline nolock_queue<T>::nolock_queue(nolock_queue &&other)
{
    m_head = other.m_head;
    m_tail = other.m_tail;
    if (m_queue)
...
template <typename T> inline size_t nolock_queue<T>::succ_idx(size_t idx)
{
    return (idx + 1) % m_size;
}

template <typename T> inline bool nolock_queue<T>::push(T &&val)
{
    size_t idx = m_tail;
    do
    {
...
template <typename T> inline bool nolock_queue<T>::pop(T &val)
{
    size_t idx = m_head;
    do
    {
